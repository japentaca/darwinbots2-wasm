...
| campo de menos de 4000 en un eje → `Add_Bot` con `Buckets(x, -1)` (`Quads.bas:91`, `:115`) | PP-01 | al menos 1 celda por eje + `SimDiag::err9_bucket_field` (añadido 2026-09-24, §15) |
...
**Qué hace el original.** `Init_Buckets` (`Quads.bas:26-27`) calcula
`NumYBuckets = Int(3000 / 4000) = 0`; `ReDim Buckets(NumXBuckets,
NumYBuckets)` deja una fila (0..0) que el bucle de `:32` nunca inicializa.
En el primer `preparerob` (`Module1.bas:36-39`: `exist = True`,
`BucketPos = (-2, -2)`), `UpdateBotBucket` clampa la celda a
`NumYBuckets - 1 = -1` (`Quads.bas:91`) y `Add_Bot` indexa `Buckets(x, -1)`
(`:115`): **error 9**, sin handler en el camino de `StartSimul`
(`main.frm:1330`; ni `StartNew_Click` ni `MDIForm_Load` tienen `On Error`).
Cargar un `.sim` así (la carga no valida el campo, `HDRoutines.bas:1143-1145`)
cae en el mismo error desde el re-registro de `Init_Buckets`
(`main.frm:1420` → `Quads.bas:56-62`), pero ese camino sí tiene handler:
`simload` hace `On Error GoTo fine` (`MDIForm1.frm:2103`) y la sim queda
a medio cargar tras el aviso. **La UI no llega ahí**: el slider va
de 1 a 25 (`OptionsForm.frm:478-479`), el 1 es F1 (9237×6928) y el menor
tamaño normal es 8000×6000 (`:4083-4097`); el otro escritor del campo,
`Form_Resize` (`main.frm:1722-1727`, solo con `WindowState = 2`), es
muerto en la práctica (Form1 sin borde y nadie lo maximiza) y ni siquiera
llama a `Init_Buckets`. Solo un archivo editado da un eje de menos de 4000. No es un `[PROBABLE BUG]` alcanzable: es un sitio de
...
`buckets.hpp`, usada por `InitBuckets` y `EnsureBuckets`; un campo no
finito o no positivo cuenta como 0 celdas) y registro en
`SimDiag::err9_bucket_field`, una vez por `Init_Buckets` que arma el sitio. Con un eje de
...
| **PP-01b** | Achicar el campo de 8000×6000 a 4000×3000 con 15 bots ya sembrados: el re-registro de `Init_Buckets` deja cada vivo registrado una vez y el contador en 1; desde una rejilla 2×1, los campos 0, −500 y NaN dan 1×1 sin recursión y suman 1 cada uno (con la rejilla ya en 1×1 no re-inicializan ni suman). | `Quads.bas:56-62` |
| **PP-02** | Campos de 4000 o más (4000², 8000×6000, F1 9237×6928, default 16000×12000): la rejilla es `Int(eje/4000)` y el contador queda en 0. | `Quads.bas:26-27`, `OptionsForm.frm:4083-4097`, `MDIForm1.frm:2479-2480` |

Los tres casos viven en `port/tests/test_buckets.cpp`. Mutation-check: sin
...
`tools/pp/smoke_campo.mjs`. Revisión de rama (agente independiente, contra el fuente): sin bugs de
código; se corrigieron citas corridas (`:26-27`, `:91`, `:32`), el matiz
de la carga con handler, `Form_Resize`, el caso de achicar en marcha
(PP-01b) y el cast de campos no finitos. Suite tras PP-01: **181 casos /
3851 aserciones** en verde en los tres modos.
