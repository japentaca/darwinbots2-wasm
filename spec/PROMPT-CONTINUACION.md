...
una sesión nueva de Claude Code abierta en el directorio del repo
...
`spec/70-CASOS-DORADOS.md` como suite de verdad (familias de extensión E4-* en
§11, E5-* en §12, E6-* en §13, E7-* en §14 y **PP-* en §15**, la de los
pendientes posteriores al plan) y la regla de oro "el fuente es la spec" para
desambiguar (el wiki no entra). Cuando un comportamiento del original es un bug,
se replica tal cual (`[PROBABLE BUG]`).
...
- **PP-01** (core, 2026-09-24): sembrar 15 algas en 4000×3000 desbordaba la
  ...
  port) re-inicializaba sin fin vía `InitBuckets` → `UpdateBotBucket`. En el
  original es un sitio de error 9 (`Add_Bot` con `Buckets(x, −1)`,
...
- **Contratos de host vigentes**: el frame del worker tiene **cabecera de 13
  ...
  capa wasm, con AbsNum distinto = slot en blanco. El campo de la página es
  libre (E1): desde PP-01 cualquier tamaño siembra y corre.
...
2. `spec/PLAN-EXTENSIONES.md §E8` — sección "Hallazgos" (origen de los
   pendientes) y `spec/70-CASOS-DORADOS.md §15` (patrón de la familia PP).
...
2. Para trabajo de **core** (`port/core/`), el ciclo es siempre: caso dorado
   ...
   **rama nueva con revisión antes de mergear** (casos nuevos en la familia PP
   de `70-CASOS-DORADOS.md §15`, o familia nueva si el tema lo pide). Antes de
   abrir el core por un tema de host, **pararse y documentar por qué**. Para
...
4. Salvaguardas numéricas de `PLAN.md`: redondeo bancario centralizado
   ...
   b As Long` declara `a` como **Variant**. `ReDim A(N, M)` va de 0 a N
   **inclusive**. Los slots que VB6 no reinicia conservan sus campos
   (`NewTeleporter`, E7). `Rnd(n)`: n<0 re-siembra, n=0 repite el último,
   n>0 avanza; `Randomize` solo reemplaza los bytes medios del estado (el byte
   bajo sobrevive). Restas de dos `Integer` dan error 6 si salen de ±32767
   aunque el resultado vaya a un `Double`.
...
(`--target dbcore.js` la compila sola). En el Bash, anteponer
`export PATH=/c/msys64/ucrt64/bin:$PATH` (si no, los `.exe` compilados a mano
no encuentran las DLL de ucrt64 y salen con 127). No hay gdb/lldb: para ubicar
una recursión o un cuelgue en nativo sirvió compilar un harness con
`-finstrument-functions` + hooks `__cyg_profile_func_enter/exit` que vuelcan
la pila de direcciones, y resolverlas con `addr2line -f -C`. **Mutation-check**:
nunca restaurar con `git checkout --` un archivo con cambios sin commitear
(se pierde el arreglo); commitear antes o usar `git stash`/copia de respaldo.
La página se sirve con `cd port && node tools/imrelay/relay.mjs` →
...
El log de la página (`#log`) va del más nuevo al más viejo; `newSim()` es
global y lee `o-fw`/`o-fh`.
...
triviales (incluidas ediciones multi-archivo), escribirlos con Write en el
scratchpad y ejecutarlos con node. Compiladores: g++/clang de MSYS2 ucrt64,
node 24.
...
modos; decisión documentada (en `PROGRESO.md` y, si toca el core, casos PP-*
nuevos en §15 con revisión de rama); smoke node de lo nuevo + verificación en
Chrome con consola limpia; prompt regenerado.

---

Regenerá este archivo con `/prompt-continuacion` al cerrar cada etapa o pendiente.
