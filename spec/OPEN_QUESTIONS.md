...
|---|---|---|---|
| Q01 | ¿Qué llamadas a `Rnd`/`Randomize` fuera de `Common.bas:rndy` ocurren **dentro** del ciclo de simulación? Alteran el orden de consumo del flujo aleatorio y por tanto los replays. Sospechosos: `Obstacles.bas` (10 usos), `main.frm` (15), `Globals.bas` (1), `HDRoutines.bas` (2). **Parcial desde A1**: dentro del tick consumen RNG `VegsRepopulate`/`aggiungirob` (`Vegs.bas:32`, `Globals.bas:411-415`), `feedvegs` (`Vegs.bas:51-52`), `feedveg2` (`Vegs.bas:282`), `BrownianForces` (`Physics.bas:113-117`), el intérprete (`DNA.bas:277`, `:1098`), `ReproduceAndKill` (`Robots.bas:1668`) y las mutaciones. **Precisado en A2**: en el intérprete son exactamente los opcodes `rnd` y `rndstore`, 1 extracción cada uno y solo cuando ejecutan (`20-VM.md §14`); `RobScriptLoad` añade 6 por bot cargado (ver Q12). Falta `Obstacles.bas` (`MoveObstacles`, B7) y `HDRoutines.bas`. | `00-INVENTARIO.md §6`, `10-CICLO.md §0.1, §13`, `20-VM.md §14` | EN CURSO |
...
| # | Pregunta | Origen | Estado |
|---|---|---|---|
| Q15 | ¿Algún subsistema fuera del intérprete escribe en `mem()` valores fuera de ±32000 (shots con tipo positivo, senses, ties)? Habilitaría el overflow `Integer` de `absstore`/`negstore` sobre −32768 (`20-VM.md §7`). | `20-VM.md §13` | ABIERTA (A3) |
