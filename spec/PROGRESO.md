...
**Próxima etapa: E8 · extras de menor valor** (`PLAN-EXTENSIONES.md §E8`):
eye designer (`frmEYE.frm`), imagen de fondo, settings del monitor RGB,
E-Grid (`EGridEnabled/Width` — `50-MUNDO.md §4` ya lo marca vestigial:
`InitEGrid` comentado en `main.frm:1503-1504`; verificar y documentar) y
tray icon (n/a en web). Pendientes que E7 dejó anotados: la ronda nueva del
port no regenera las formas desde `xObstacle` (`main.frm:1353`, límite de
E5), y la orquestación de liga de `MDIForm1.frm:2536-2790` sigue fuera
(orquestación local por disco, sin relación con el transporte).
...
- **Plan de extensiones aprobado por el usuario (2026-08-26)**: cubrir el resto
  ...
  superficie del original), E7 Internet **host + core** ✅, E8 extras). Orden recomendado: E8. Hallazgo que lo ordena: el core ya implementa
