...
**Próxima etapa: E7 · Internet / torneo distribuido**
(`PLAN-EXTENSIONES.md §E7`) — **capa host (transporte)**:

- La E/S por búferes ya funciona entre sims desde M8
  (`db_sim_tp_outbox_take` / `db_sim_tp_inbox_push`, `Teleport.bas`): falta
  el **transporte real entre navegadores** — la capa ⚙ de `50-MUNDO.md §5`.
  Diseño abierto: servidor simple (WebSocket) o WebRTC. Decidir al llegar,
  con `NetEvent.frm` y `provvisorio.bas` a la vista.
- Lo que E6 dejó anotado como de E7: `writeIMdata` (`main.frm:3134`, el JSON
  de stats cada 200 ciclos con `strSimStart`), `InternetSpecies`
  (`provvisorio.bas:19-21`, hasta 500 especies remotas — el `SetValues` de
  `grafico.frm:3735-3742` ya las contempla como serie de chart) y el modo
  eco-IM (`y_eco_im`, que contamina el tag con el nrg — B8-3).
- La orquestación de liga (`MDIForm1.frm:2536-2790`) que E5 dejó fuera.

Después de E7: **E8** (extras de menor valor: eye designer, imagen de fondo,
monitor RGB, E-Grid, tray icon).
...
- **Plan de extensiones aprobado por el usuario (2026-08-26)**: cubrir el resto
  ...
  superficie del original), E7 Internet, E8 extras). Orden recomendado: E7 → E8. Hallazgo que lo ordena: el core ya implementa
